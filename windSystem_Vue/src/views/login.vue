<template>
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm"  ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      <h2 class="title">风电机组关键设备全寿命周期管理系统</h2><br/>
      <h3 class="title1">登录</h3>
      <el-select v-model="value" placeholder="请选择登录的身份">
    <el-option
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
          ...
          //   this.$router.push({ path: '/monitor/Monitor' })
          // }, 1000)
          let that = this;
          
          
            axios.post('http://localhost:8080/rightmanages/login',this.ruleForm)
            .then(function (response) {
              ...
              if(response.data.state == 200){
                ...
                if(that.value == '选项1'){
                  that.$store.commit('saveIsAdmin', true)
                  console.log("'saveIsAdmin', 'true'")
                }else{
                  that.$store.commit('saveIsAdmin', '')
                }
                that.$router.push({ path: '/monitor/Monitor' })
                that.logining = false
