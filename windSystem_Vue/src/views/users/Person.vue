<template>
    <div>
      ...
      <el-table
        :data="tableData"
        border>
        <el-table-column prop="pid" label="记录编号" width="150"></el-table-column>
        ...
        <el-table-column prop="content" label="维修内容" width="150"></el-table-column>
      ...
      <el-dialog :title="title" :visible.sync="editFormVisible" width="30%" @click="closeDialog">
        <el-form label-width="120px" :model="editForm"  ref="editForm">
          <el-form-item  prop="pid" label="记录编号">
            <el-input size="small" v-model="editForm.pid" auto-complete="off" placeholder="请输入记录编号"></el-input>
          </el-form-item>
          ...
          <el-form-item  prop="content" label="维修内容">
            <el-input size="small" v-model="editForm.content" auto-complete="off" placeholder="请输入维修内容"></el-input>
          </el-form-item>
          ...
          <el-form-item prop="comment" label="工作票编号">
  ...
  export default {
    ...
    methods: {
      ...
      search() {
     var that=this;
        axios.get(`http://localhost:8080/personmanages/${that.formInline.name}`
        )
        .then(function (response) {
          that.tableData=[]
          console.log(response); // 前台调试能够看到获取到数
      ...
      if(this.status === "update"){
        ...
        if (valid) {
                var that=this;
        axios.put('http://localhost:8080/personmanages',
        this.editForm)
        .then(function (response) {
      ...
      if(this.status === "add"){
        ...
        axios.post('http://localhost:8080/personmanages',
        this.editForm)
        .then(function (response) {
          console.log(response); // 前台调试能够看到获取到数据了
          that.tableData=response.data.data
      ...
      this.$confirm('确定要删除吗?', '信息', {
        ...
        .then(() => {
          axios.delete(`http://localhost:8080/personmanages/${row.pid}`).then(function (response) {
          console.log(response); // 前台调试能够看到获取到数据了
